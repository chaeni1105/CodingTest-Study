...
for i in range(n_row) :
    for j in range(m_col) :
        ...
        if village[i][j] == 1:
            ...
            num.append(cnt) # num리스트에 몇개의 1이 인접해있는 집인지 저장한다.
            
for i in range(len(num)): # 창고+집의 총 개수만큼 반복한다.
    if num[i] == 1: # 만약 한 개의 1로 구성된 곳이라면 집이아니라 창고이다.
        changgo += 1 # 창고 +1
        
home = len(num) - changgo # 창고+집의 총 개수에서 창고를 뺀 수는 집의 갯개수이다.
